...
static int ubus_msg_writev(int fd, struct ubus_msg_buf *ub, int offset)
{
	...
	if (offset < sizeof(ub->hdr)) {
		...
		hdr.seq = cpu_to_le16(ub->hdr.seq);
		hdr.peer = cpu_to_le32(ub->hdr.peer);
...
retry:
	if (!sock->eof && cl->pending_msg_offset < sizeof(cl->hdrbuf)) {
		...
		cl->hdrbuf.hdr.seq = le16_to_cpu(cl->hdrbuf.hdr.seq);
		cl->hdrbuf.hdr.peer = le32_to_cpu(cl->hdrbuf.hdr.peer);
