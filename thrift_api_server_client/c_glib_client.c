...
int main (void)
{
  ...
  if (!error && calculator_if_get_log_type1 (client, &sum, &error)) {
    printf ("getlogtype=%d\n", sum);
  }
  if (!error && calculator_if_set_log_type1 (client, &sum,2, &error)) {
    printf ("getlogtype=%d\n", sum);
  }
  ...
  if (!error && calculator_if_get_test_report (client, &sum, &error)) {
    printf ("get test report=%d\n", sum);
  }

  if (!error && calculator_if_set_test_report (client, &sum, 1, &error)) {
    printf ("set test report=%d\n", sum);
  }
