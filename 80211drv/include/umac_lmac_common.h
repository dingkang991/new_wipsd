 ...
 */
/* Modified by QTS x00042 20141009 for multiBSS mac management */
#if 0
...
#define ATH_SET_VAP_BSSID(bssid, hwbssid, id)                        \
            do {                                                             \
                if (id) {                                                    \
                    u_int8_t hw_bssid = 0; \
                    (bssid)[IEEE80211_ADDR_LEN - 1] &= ~((ATH_BCBUF - 1));                   \
                    (bssid)[IEEE80211_ADDR_LEN - 1] |= (((id) + hw_bssid) & (ATH_BCBUF - 1));     \
                }                                                            \
            } while(0)

#endif
/* End modified by QTS 20141009 for multiBSS mac management */
