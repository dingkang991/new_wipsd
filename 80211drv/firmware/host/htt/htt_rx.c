...
{
    int i;
    unsigned char *buf = adf_nbuf_data(msdu);
    ...
    for (i = 0; i < len; i++) {
        ...
        for (i = 0; i < 48; i++) {
            printk("0x%x ", (unsigned char)msdu->cb[i]);
            if (i && (i % 8 == 0))
                printk("\n");
        }

        printk("\n++++++++++++++++++++++++++++++++++++++++++++\n");
        printk("skb Tail: 0x%p, End: 0x%p, Head: 0x%p\n",
