...
#else
/****QTS:X00042*****/
int mac_addr_verify(u_int8_t *macaddr)
{
	int iRet = 0;
	int i;
...
static inline HAL_STATUS ar9300_init_mac_addr(struct ath_hal *ah)
{
	...
	ret = ath9k_hw_macaddr_file("/tmp/.phy0_mac", buf);
	if(ret){
		printk(KERN_ERR "PHY0 read mac failed\t\n");
		OS_MEMCPY(ahp->ah_macaddr,mac_default,IEEE80211_ADDR_LEN);
		goto OUT;
	}

	ret = ath9k_hw_mac_transfer(mac, buf);
	if(ret){
...
OUT:
	...
	if (IS_ERR(pfp))
	{
		printk("einfo block open failed!\n");
		return HAL_EEBADMAC;
	}
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	...
	ifreadlen = kernel_read(pfp,WLAN_MAC_OFFSET,(char *)buf,IEEE80211_ADDR_LEN);
	if( ifreadlen < IEEE80211_ADDR_LEN)
	{
		iRet = 1;
	}
	else
	{
		iRet = mac_addr_verify(buf);
	}
	if(iRet)
	{
		OS_MEMCPY(ahp->ah_macaddr,mac_default,IEEE80211_ADDR_LEN);
	...
	{
		OS_MEMCPY(ahp->ah_macaddr,buf,IEEE80211_ADDR_LEN);
	}
	filp_close(pfp, NULL);
#endif
	return HAL_OK;
