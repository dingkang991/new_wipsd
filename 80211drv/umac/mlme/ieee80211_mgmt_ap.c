...
wlan_send_mgmt(wlan_if_t vaphandle, u_int8_t *macaddr, u_int8_t *mgmt_frm, u_int32_t len)
{
    ...
    else
    {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,
                              "%s-%d: i_addr1=%02x:%02x:%02x:%02x:%02x:%02x i_addr2=%02x:%02x:%02x:%02x:%02x:%02x i_addr3=%02x:%02x:%02x:%02x:%02x:%02x\n", 
                              __func__, __LINE__, 
        ...
        ni = ieee80211_tmp_node(vap, wh->i_addr1);
        if (ni == NULL) {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME, "%s: can't alloc tmp node\n",  __func__);

            return -EINVAL;
        }

        memcpy(ni->ni_macaddr, wh->i_addr1, sizeof(ni->ni_macaddr));
        memcpy(ni->ni_bssid, wh->i_addr3, sizeof(ni->ni_bssid));
        
        ieee80211_send_setup(vap, ni, wh,
                             type | subtype,
                             wh->i_addr2, ieee80211_node_get_macaddr(ni), ieee80211_node_get_bssid(ni));
