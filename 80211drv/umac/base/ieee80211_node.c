...
extern int rt_assoc_notify(wlan_if_t vap, u_int8_t *macaddr, u_int8_t aflag);
...
#undef N
    /* 2g/5g delete */
    /* win10在省电模式会发送null data，ap收到null data之后会创建IEEE80211_NODE_TEMP节点,应加判断IEEE80211_NODE_TEMP */
    if ((ni->ni_vap) && (ni->ni_vap->iv_bss != ni) && 
        (ni->ni_vap->iv_bss == ni->ni_bss_node) && 
        !(ni->ni_flags & IEEE80211_NODE_TEMP))
    {
        rt_assoc_notify(ni->ni_vap, ni->ni_macaddr, RT_ACTION_DISASSOC_NOTIFY);
    }
    OS_FREE(ni);
}
...
ieee80211_dup_bss(struct ieee80211vap *vap, const u_int8_t *macaddr)
{
    ...
    if (ni != NULL) {
        ...
        if (!IEEE80211_ADDR_EQ(vap->iv_myaddr, ni->ni_macaddr))
            rt_assoc_notify(ni->ni_vap, ni->ni_macaddr, RT_ACTION_ASSOC_NOTIFY);
    }
