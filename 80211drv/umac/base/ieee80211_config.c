...
#include "../../firmware/host/wlan/txrx/include/ol_txrx_internal.h"
...
#undef DEFAULT_MIN_SCAN_DWELL_TIME        
    ...
    case IEEE80211_VHT80_RATEMASK:
        ...
        break;
    case IEEE80211_PORTAL_ID:
        if (val <=0 || val >255)
        {
            retv = -EINVAL;
            break;
        }
        vap->iv_portal_id = (u_int8_t)val;
        break;
    case IEEE80211_PORTAL_ENABLE:
        if (val > 1 || val < 0)
        {
            retv = -EINVAL;
            break;
        }
        vap->iv_portal_enable = (u_int8_t)val;
        break;
    case IEEE80211_IS_AUTO_PORTAL:
        if (val > 1 || val < 0)
        {
            retv = -EINVAL;
            break;
        }
        vap->iv_is_auto_portal = (u_int8_t)val;
        break;
    case IEEE80211_NO_AVOID_BLOCK:
        if (val > 1 || val < 0)
        {
            retv = -EINVAL;
            break;
        }
        vap->iv_no_avoid_block = (u_int8_t)val;
        break;
    case IEEE80211_TXRX_PRINT_LEVEL:
        if ((val < TXRX_PRINT_LEVEL_FATAL_ERR) || val > 
        ...
        ol_txrx_print_level_set(val);
        break;
    case IEEE80211_VAP_MONITOR:
        if (val > 1 || val < 0)
        {
            retv = -EINVAL;
            break;
        }
        vap->iv_vap_monitor = (u_int8_t)val;
        break;
    default:
        break;
    }
...
#endif /* ATH_PERF_PWR_OFFLOAD && QCA_SUPPORT_RAWMODE_PKT_SIMULATION */
    case IEEE80211_PORTAL_ID:
        val = vap->iv_portal_id;
        break;
    case IEEE80211_PORTAL_ENABLE:
        val = vap->iv_portal_enable;
        break;
    case IEEE80211_TXRX_PRINT_LEVEL:
        val = ol_txrx_print_level_get();
        break;
    case IEEE80211_IS_AUTO_PORTAL:
        val = vap->iv_is_auto_portal;
        break;
    case IEEE80211_NO_AVOID_BLOCK:
        val = vap->iv_no_avoid_block;
        break;
    case IEEE80211_VAP_MONITOR:
        val = vap->iv_vap_monitor;
        break;
    default:
        break;
    }
