...
#define	IEEE80211_NODE_DHCP_BEGIN			0x02
#define	IEEE80211_NODE_DHCP_DONE			0x04
#define	IEEE80211_NODE_PORTAL_DONE			0x08
#define	IEEE80211_NODE_IP_ERROR 			0x20
#define	WLAN_STA_DHCP_ACK_REPORT 			0x40
...
#define IEEE80211_NODE_IPV6_MAX 4
    ...
    u_int32_t   ps_state;
	u_int32_t	ni_haddr;		/*station host address*/
	u_int32_t   ni_ostype;		/*station os type*/
#define IEEE80211_NODE_DEVNAME_MAX 128
	char		ni_devname[IEEE80211_NODE_DEVNAME_MAX];		/*station device name*/
...
#define TRACENODE(_ni, _func, _line) do {                             \
    int index = atomic_inc_return(&(_ni)->index) & (NUM_TRACE_BUF - 1); \
    (_ni)->func[index] = _func;                                       \
    (_ni)->line[index] = _line;                                       \
    atomic_set(&(_ni)->refcnt[index], ieee80211_node_refcnt(_ni));\
} while (0)
#endif
