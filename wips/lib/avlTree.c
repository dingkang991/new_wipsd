...
Node* avltree_insert(AVLTree tree, Type key)
{
    if (tree == NULL) 
    {
        ...
        if (tree==NULL)
        {
            log_error("ERROR: create avltree node failed!\n");
            return NULL;
        }
    }
...
void print_avltree(AVLTree tree, Type key, int direction)
{
    if(tree != NULL)
    {
        if(direction==0)    // tree是根节点
            log_debug("%2d is root\n", tree->key, key);
        else                // tree是分支节点
            log_debug("%2d is %2d's %6s child\n", tree->key, key, direction==1?"right" : "left");
...
/*
    ...
    log_debug("== 高度: %d", avltree_height(root));
