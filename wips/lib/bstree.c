...
static struct bstree_node *bstree_search_recursive(struct bstree_node *root,
												   void *needle,  
...
static struct bstree_node *alloc_bstree_node(void)
{
	...
	r=malloc(sizeof(struct bstree_node));
...
//	printf("new bstree node:%p data:%p\n",node,data);
	node->data=data;
				 ...
				 void (*free_data)(void *))
{
	...
	bstree_free(node->left,free_data);
	bstree_free(node->right,free_data);
				  ...
				  void (*del)(void *))
{
	...
	if(toDelete) /* we found the guy */
	{
		struct bstree_node *parent = toDelete->parent;
		/* in case the guy is alone */
		if(!toDelete->left && !toDelete->right)
		{
			/* it has a parent: it is not the root,
			   but we have to tell his parent that he was freed */
			if(parent)
			{
			...
			else /* the guy has no parent: it is the root itself */
				*root = NULL; /* reset the root */

			/* delete the data it contained */
			if(del)
				(del)(toDelete->data);
			free(toDelete);
