...
struct hash_control *
hash_new_bysize (unsigned long new_size)
{
  unsigned long size;
  unsigned long alloc;
  struct hash_control *ret;
...
void *
hash_find (struct hash_control *table, const char *key, int key_len)
{
  struct hash_entry *p;
  ...
  if(key_len <= 0)
	p = hash_lookup (table, key, strlen (key), NULL, NULL);
  	...
  	p = hash_lookup (table, key, key_len, NULL, NULL);
...
void *
hash_delete (struct hash_control *table, const char *key, int key_len, int freeme)
{
  struct hash_entry *p;
  struct hash_entry **list;
  void* tmp;
  ...
  if(key_len == 0)
      ...
      p = hash_lookup (table, key, key_len, &list, NULL);
...
main ()
{
  ...
  number = 0;
  h = 0;
  WIPSD_DEBUG ("type h <RETURN> for help\n");
  for (;;)
    {
      WIPSD_DEBUG ("hash_test command: ");
      gets (answer);
      command = answer[0];
	...
	{
	case '#':
	  WIPSD_DEBUG ("old hash table #=%d.\n", number);
	  whattable ();
	  break;
	case '?':
	  for (pp = hashtable; pp < hashtable + TABLES; pp++)
	    {
	      WIPSD_DEBUG ("address of hash table #%d control block is %xx\n",
		      pp - hashtable, *pp);
	    }
	  break;
	case 'a':
	...
	case 'd':
	  ...
	  break;
	case 'f':
	  p = hash_find (h, name = what ("symbol"), 0);
	  WIPSD_DEBUG ("value of \"%s\" is \"%s\"\n", name, p ? p : "NOT-PRESENT");
	  break;
	case 'h':
	  WIPSD_DEBUG ("# show old, select new default hash table number\n");
	  WIPSD_DEBUG ("? display all hashtable control block addresses\n");
	  WIPSD_DEBUG ("a apply a simple display-er to each symbol in table\n");
	  WIPSD_DEBUG ("d die: destroy hashtable\n");
	  WIPSD_DEBUG ("f find value of nominated symbol\n");
	  WIPSD_DEBUG ("h this help\n");
	  WIPSD_DEBUG ("i insert value into symbol\n");
	  WIPSD_DEBUG ("j jam value into symbol\n");
	  WIPSD_DEBUG ("n new hashtable\n");
	  WIPSD_DEBUG ("r replace a value with another\n");
	  WIPSD_DEBUG ("s say what %% of table is used\n");
	  WIPSD_DEBUG ("q exit this program\n");
	  WIPSD_DEBUG ("x delete a symbol from table, report its value\n");
	  break;
	case 'i':
	  p = hash_insert (h, name = what ("symbol"), 0, value = what ("value"));
	  if (p)
	    {
	      WIPSD_DEBUG ("symbol=\"%s\"  value=\"%s\"  error=%s\n", name, value,
		      p);
	    }
	  break;
	case 'j':
	...
	case 'x':
	  ...
	  break;
	default:
	  WIPSD_DEBUG ("I can't understand command \"%c\"\n", command);
	  break;
	}
    }
}
...
whattable ()
{
  for (;;)
    {
      WIPSD_DEBUG ("   what hash table (%d:%d) ?  ", 0, TABLES - 1);
      gets (answer);
      ...
      if (number >= 0 && number < TABLES)
	{
	  ...
	  if (!h)
	    {
	      WIPSD_DEBUG ("warning: current hash-table-#%d. has no hash-control\n", number);
	    }
	  return;
	}
      else
	{
	  WIPSD_DEBUG ("invalid hash table number: %d\n", number);
	}
    }
}
